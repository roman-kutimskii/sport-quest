...
DIR=/opt/sport-quest
rsync -az -e "${RSYNC_RSH:-ssh}" --delete --exclude-from=.dockerignore --exclude=.env.prod ./ "$HOST:$DIR/"
${RSYNC_RSH:-ssh} "$HOST" "cd $DIR && docker compose -f compose.prod.yml --env-file .env.prod up -d --build --remove-orphans \
