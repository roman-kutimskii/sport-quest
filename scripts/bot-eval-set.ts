...
export const EVAL_SET: EvalCase[] = [
  // clear reports
  { text: "Пробежал 5 км 🍂", mediaKinds: ["photo"], expect: { is_report: true, band: "save", activity_type: "run", date: "2026-09-10" } },
  { text: "Сегодня зал, ноги 💪", expect: { is_report: true, band: "save", activity_type: "gym" } },
  { text: "Йога 30 минут перед сном", expect: { is_report: true, band: "save", activity_type: "yoga" } },
  { text: "Проехал на веле 20 км до работы и обратно", expect: { is_report: true, band: "save", activity_type: "bike" } },
  { text: "Бассейн, 1 км", expect: { is_report: true, band: "save", activity_type: "swim" } },
  { text: "Зарядка дома сделана ✅", expect: { is_report: true, band: "save", activity_type: "home" } },
  { text: "Турники во дворе, 5 подходов подтягиваний", expect: { is_report: true, band: "save", activity_type: "workout" } },
  { text: "Прогулка 12 000 шагов", expect: { is_report: true, band: "save", activity_type: "walk", steps: 12000 } },
  { text: "14 532 шага за день", expect: { is_report: true, steps: 14532 } },
  { text: "12к шагов сегодня", expect: { is_report: true, steps: 12000 } },
  { text: "Отчёт: бег 6 км, 11 200 шагов", expect: { is_report: true, band: "save", activity_type: "run", steps: 11200 } },
  { text: "", mediaKinds: ["photo"], expect: { is_report: true } }, // photo without caption — any band, the model decides from the image
  { text: "Час в тренажёрке", mediaKinds: ["photo"], expect: { is_report: true, band: "save", activity_type: "gym" } },
  { text: "Сходила на пилатес", expect: { is_report: true, band: "save" } },
  { text: "Погоняли в футбол с ребятами", expect: { is_report: true, band: "save" } },

  // relative dates
  { text: "Вчера пробежала 3 км, забыла написать", expect: { is_report: true, activity_type: "run", date: "2026-09-09" } },
  { text: "Позавчера был зал", expect: { is_report: true, activity_type: "gym", date: "2026-09-08" } },
  { text: "В понедельник бегал 5 км", expect: { is_report: true, activity_type: "run", date: "2026-09-07" } },
  { text: "Утром сделал зарядку", expect: { is_report: true, activity_type: "home", date: "2026-09-10" } },
  { text: "8 сентября — велик 15 км", expect: { is_report: true, activity_type: "bike", date: "2026-09-08" } },

  // explicit bingo
  { text: "Лифтофобия: 9 этаж пешком 🪜", mediaKinds: ["photo"], expect: { is_report: true, bingo_key: "stairs", bingo_explicit: true } },
  { text: "Бинго «Ранняя пташка»: пробежка в 6:40", mediaKinds: ["photo"], expect: { is_report: true, activity_type: "run", bingo_key: "early", bingo_explicit: true } },
  { text: "Йога в тёплых носках — уютный дзен 🧘", mediaKinds: ["photo"], expect: { is_report: true, activity_type: "yoga", bingo_key: "zen", bingo_explicit: true } },
  { text: "Заслуженный чай после пробежки ☕️", mediaKinds: ["photo"], expect: { is_report: true, bingo_key: "tea", bingo_explicit: true } },
  { text: "Спорт-коллаб с Машей: прогулка 8 км", mediaKinds: ["photo"], expect: { is_report: true, activity_type: "walk", bingo_key: "collab", bingo_explicit: true } },
  { text: "Пробежка в темноте с фонариком — ночной дозор 💡", mediaKinds: ["photo"], expect: { is_report: true, activity_type: "run", bingo_key: "night", bingo_explicit: true } },
  ...
  { text: "1 сентября пробежал 5 км", expect: { is_report: true, activity_type: "run", date: "2026-09-01" } }, // pre-quest → the worker drops it
];
