...
export type Mention = {
  ...
  name: string;
  /** Linked to an active quest participant. */
  participant: boolean;
};
...
export type PromptContext = {
  ...
  imageCount: number;
  forwarded: boolean;
  /** Users mentioned in the message; empty when there are none. */
  mentions?: Mention[];
};
...
${activities}
Если автор сделал несколько разных активностей за день («утром бег, вечером йога») — перечисли все ключи; иначе один. Число шагов, названное рядом с другой активностью («бег 6 км, 11 200 шагов») или взятое со скриншота шагомера, НЕ добавляет "walk" — оно идёт в steps. Правило: «прогулка» или «10 000+ шагов» → "walk"; силовая в зале → "gym"; турники/отжимания → "workout"; зарядка дома → "home"; спорт, которого нет в списке (лыжи, теннис, футбол, танцы, скалолазание…) → "other".
...
Схема ответа (все ключи обязательны):
{
  ...
  "steps": integer | null,     // количество шагов: из текста или со скриншота шагомера
...
4. steps — число шагов. Бери его из двух источников:
   а) текст: «12 000 шагов», «15к шагов» → 15000, «шаги: 8543»;
   б) приложенный СКРИНШОТ ШАГОМЕРА (Mi Fitness, Health, Шаги, Google Fit, Samsung Health и т. п.), где видно дневное число шагов — обычно крупное число рядом со словом «шаги»/«steps» или столбчатым графиком по часам.
   Из скриншота бери число ТОЛЬКО если оно подписано как шаги и это итог за день. Если на скриншоте видна дата и она не совпадает с датой активности из этого сообщения — steps = null.
...
export function buildUserParts(ctx: PromptContext, images: { mime: string; data: Buffer }[]): LlmPart[] {
  ...
  const lines = [
    ...
    `Упомянутые: ${ctx.mentions?.length ? ctx.mentions.map((m) => `${m.ref} (${m.name}, ${m.participant ? "участник квеста" : "НЕ участник квеста"})`).join("; ") : "нет"}`,
    `Текст сообщения:`,
    ctx.text ? `"""\n${ctx.text}\n"""` : "(без текста)",
  ];
