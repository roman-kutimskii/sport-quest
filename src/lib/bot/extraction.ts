...
function normalizeRaw(raw: unknown): unknown {
  ...
  for (const k of ["date", "activity_type", "steps", "bingo_key"]) if (nullish(o[k])) o[k] = null;
...
/**
 * Zod parse plus enum checks against code constants: unknown `activity_type` → null; `bingo_key`
...
export function coerceExtraction(raw: unknown, openBingoKeys: string[]): Extraction {
  ...
  const activity = e.activity_type?.trim().toLowerCase() ?? null;
  const activity_type = activity && ACTIVITY_TYPES.some((t) => t.key === activity) ? activity : null;
...
${activities}
Правило: «прогулка» или «10 000+ шагов» → "walk"; силовая в зале → "gym"; турники/отжимания → "workout"; зарядка дома → "home"; спорт, которого нет в списке (лыжи, теннис, футбол, танцы, скалолазание…) → "other".
...
Схема ответа (все ключи обязательны):
{
  ...
  "activity_type": string | null, // ключ из списка выше или null
...
4. steps: только если в тексте есть число шагов («12 000 шагов», «15к шагов» → 15000, «шаги: 8543»). Километры, минуты, этажи — не шаги. Если шагов нет — null. Шаги могут быть отчётом сами по себе (activity_type может быть null).
